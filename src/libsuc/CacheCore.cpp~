...
*CacheAssoc<State, Addr_t, Energy>::findLine2ReplaceWithoutReorder(Addr_t addr, bool ignoreLocked)
{
    Addr_t tag    = calcTag(addr);
    Line **theSet = &content[calcIndex4Tag(tag)];
    ...
    Line **setEnd = theSet + assoc;
    Line **nxlruLineFree=0;
    Line **coherentInvalidLineFree=0;

    // Start in reverse order so that get the youngest invalid possible,
    // and the oldest isLocked possible (lineFree)
    int unLockedValidLineCount = 0;
    bool invalidLineFreeFound = false;
    bool coherentInvalidLineFreeFound = false;

    {
        ...
        while(l >= theSet) {
            ...
            if (!(*l)->isValid()) {
                lineFree = l;
                invalidLineFreeFound = true;
                if ((*l)->getPrevTag() == tag) { //for cache coherence
                    coherentInvalidLineFree = l;
                    coherentInvalidLineFreeFound = true;
                }
            } else if (lineFree == 0 && !(*l)->isLocked()) {
                lineFree = l;
                ++unLockedValidLineCount;
            } else if (unLockedValidLineCount == 1 && !(*l)->isLocked()) {
                nxlruLineFree = l;
                ++unLockedValidLineCount;
            }
    ...
    bool nxlruEntryToBeUsed = (invalidLineFreeFound == false && policy == NXLRU && unLockedValidLineCount > 1);
    Line *tmp = nxlruEntryToBeUsed ? *nxlruLineFree : coherentInvalidLineFreeFound ? *coherentInvalidLineFree : *lineFree;
...
*CacheAssoc<State, Addr_t, Energy>::findLine2Replace(Addr_t addr, bool ignoreLocked)
{
    ...
    bool nxlruEntryToBeUsed = (invalidLineFreeFound == false && policy == NXLRU && unLockedValidLineCount > 1);
    Line *tmp = nxlruEntryToBeUsed ? *nxlruLineFree : *lineFree;
    
    {
        Line **l = nxlruEntryToBeUsed ? nxlruLineFree : lineFree;
        while(l > theSet) {
            Line **prev = l - 1;
            *l = *prev;;
            l = prev;
...
*CacheDM<State, Addr_t, Energy>::findLine2ReplaceWithoutReorder(Addr_t addr, bool ignoreLocked)
{
...
*CacheDMSkew<State, Addr_t, Energy>::findLine2ReplaceWithoutReorder(Addr_t addr, bool ignoreLocked)
{
    return findLine2Replace(addr, ignoreLocked);
}
