...
BPNotTaken::BPNotTaken(int32_t i, int32_t fetchWidth, const char *section)
...
PredType  BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
    ...
    int32_t address = inst->getAddr();
    bool correctGuess = inst->calcNextInstID() == oracleID;
 
    return correctGuess ? CorrectPrediction : MissPrediction;
}
